...
void ADC::print1byte(uint8_t input, struct iio_channel_info *info)
{
    /*
...
void ADC::print2byte(uint16_t input, struct iio_channel_info *info)
{
...
void ADC::print4byte(uint32_t input, struct iio_channel_info *info)
{
    /* First swap if incorrect endian */
    if (info->be)
        input = be32toh(input);
...
void ADC::print8byte(uint64_t input, struct iio_channel_info *info)
{
    /* First swap if incorrect endian */
    if (info->be)
        input = be64toh(input);
 ...
 * @num_channels:	number of channels
 **/
void ADC::process_scan(char *data,
...
int ADC::enable_disable_all_channels(char *dev_dir_name, int enable)
{
...
int ADC::enable_disable_one_channel(char *dev_dir_name, int enable, int channel_num)
{
    const struct dirent *ent;
    char scanelemdir[256];
    DIR *dp;
    ...
    sprintf(channel_name, "%d_%s", channel_num, "en");
    ...
    while (ent = readdir(dp), ent)
    {
        if (iioutils_check_suffix(ent->d_name, channel_name))
        {
            printf("%sabling: %s\n",
                   enable ? "En" : "Dis",
                   ent->d_name);
...
int ADC::init()
{
    ...
    if (!device_name)
    {
        device_name = (char *)malloc(IIO_MAX_NAME_LENGTH);
        if (!device_name)
        {
            ret = -ENOMEM;
            cleanup();
    ...
    else if (trig_num >= 0)
    {
        ...
        trigger_name = (char *)malloc(IIO_MAX_NAME_LENGTH);
    ...
    if (!num_channels && autochannels == AUTOCHANNELS_DISABLED)
    {
        ...
        fprintf(stderr, "Enable channels manually in " FORMAT_SCAN_ELEMENTS_DIR "/*_en or pass -a to autoenable channels and "
                        "try again.\n",
                dev_dir_name);
        ret = -ENOENT;
        cleanup();
