...
if (dev_num < 0)
{
  ...
  int ADC::size_from_channelarray(struct iio_channel_info *channels, int num_channels)
  {
    ...
    while (i < num_channels)
    {
      ...
      channels[i].location = bytes - bytes % channels[i].bytes + cha nnels[i].bytes;
    ...
    int AD
    C::EnableDi sableChann
    els(int ena ble)
        { const struc
          t dirent *e nt;char s canelemdir
          [256];
          DIR *dp;
        int ret;
        char * buf_dir_name = NULL;
        ret = build_channel_array(dev_di r_
          name
  ...
  if (ret < 0) {
    fprintf(stderr, "Failed to enable buffer: %s\n", strerror(-ret));
  }
...
void ADC::read_adc(float *values) {
  ...
  for (i = 0; i < read_size / scan_size; i++) {
    for (k = 0; k < num_channels; k++) {
      float adc_val = print2byte(
          *(uint16_t*) ((data + scan_size * i) + channels[k].location),
          &channels[k]);
 ...
 * @num_channels:	number of channels
 **/
float* ADC::process_scan(char *data, struct iio_channel_info *channels,
                         int num_channels) {
  int k;
  float *val_read;
  for (k = 0; k < num_channels; k++)
