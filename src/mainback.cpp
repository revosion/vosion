...
class Record {
 public:
  ...
  ;
  Json to_json() const {
    return Json::object { { "n", name }, { "u", unit }, { "v", value }, };
  }
};
...
int main(int argc, char *argv[]) {
  ...
  const string channel_id = config["cloud_connection"]["channel_id"]
      .string_value();
  const string thing_id = config["cloud_connection"]["thing_id"].string_value();
  const string thing_key =
      config["cloud_connection"]["thing_key"].string_value();
  const int max_buffered_msgs = config["cloud_connection"]["max_buffered_msgs"]
      .int_value();
  const string persist_dir = config["cloud_connection"]["persist_dir"]
      .string_value();
  ...
  set_frequency_msg["param_addr"] =
      config["commands"]["set_frequency"]["param_addr"];
  set_frequency_msg["operation"] =
      config["commands"]["set_frequency"]["operation"];
  ...
  try {
    ...
    while (0 == sigval) {
      ...
      auto ms = chrono::duration_cast < std::chrono::milliseconds
          > (chrono::system_clock::now().time_since_epoch()).count();
      cout << ms << endl;
      Json::object base_json = Json::object { { "bn", "pump1" }, { "bt", double(
          ms) }, { "bu", "A" }, { "bver", 1 }, { "n", "sim" }, { "u", "V" }, };
